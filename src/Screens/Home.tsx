...
import { eventInfo, highlights, eventNotification, EVENT_NAME } from '../data';
...
function Home() {
  ...
  useEffect(() => {
    const hasSeenEvent = localStorage.getItem(EVENT_NAME);
    if (!hasSeenEvent) setShowEventModal(true);
  }, []);

  const handleCloseModal = () => {
    localStorage.setItem(EVENT_NAME, "true");
    ...
    <div className='min-h-screen'>
      ...
      <div ref={homeRef}>
        ...
        <div className="mx-auto w-screen px-4 flex flex-col items-center">
          ...
          <p className="mt-3 text-gray-700 max-w-3xl text-sm">
      ...
      <div ref={aboutRef}>
        <div className="max-w-7xl mx-auto px-4 py-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {highlights.map(({ title, description, image, icon: Icon, iconColor }, index) => (
              <div
                key={index}
